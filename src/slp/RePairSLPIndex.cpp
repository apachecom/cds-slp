...
namespace cds_static
{
	...
	uint 
	RePairSLPIndex::calc_len(const uint &X, rvect &D, lvect &L, const int & t) {
		if (X < t) return 1;
		if (L[X-t] != 0)
			return L[X-t];
	...
	void 
	RePairSLPIndex::build(uchar *text, uint length, uint sampling_length, char *filename, std::fstream &in_grammar, std::fstream & in_first_rule){
...
//            std::cout<<std::endl;
	...
	void RePairSLPIndex::buildSamplingsOwn(uint qsampling) {
            {
                // *** GRAMMAR SAMPLING
                // *******************
                uint DELTA_SAMP = 4;
	...
	void 
	RePairSLPIndex::extractCharsDirectNoSamplesOwn(uint rule, uint len, uint chars, uchar *sample){
	...
	int
	RePairSLPIndex::load(const int & AA,char *filename, RePairSLPIndex **rpi)
	{
		*rpi = new RePairSLPIndex();
		(*rpi)->filename = string(filename);
