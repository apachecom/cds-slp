...
namespace cds_static{
	...
	factorization::factorization(uint *list,uint l_Length){
		...
		levels = new uint8_t[levelsIndex[nLevels]/2]; 
		...
		//If cont is odd, then is put in the upper part of the uint8_t(<<BASE_BITS)
		//in other case:
		for(i=0;i<listLength;i++){
			value = list[i];
			j= nLevels-1;
			while(j>=0){
				if(value >= TABLEBASE[j]){
					...
					for(k=0; k<j; k++){
						if(cont[k]%2)
							levels[cont[k]/2]=levels[cont[k]/2]+(((uint8_t)newvalue%BASE)<<BASE_BITS);
						else
							levels[cont[k]/2]=(uint8_t)newvalue%BASE;
						cont[k]++;
						newvalue = newvalue/BASE;
					}
					if(cont[j]%2)
						levels[cont[j]/2] = levels[cont[j]/2]+(((uint8_t)newvalue%BASE)<<BASE_BITS);
					else
						levels[cont[j]/2] = (uint8_t)newvalue%BASE;	
					cont[k]++;
					if(j<nLevels-1)
		...
		bS = new BitSequenceRG(bits_BS, bits_BS_len, 3); 	//(bitarray, length, factor)
		// *** CAMBIO FINAL		bS = new BitSequence375(bits_BS, bits_BS_len); 	//(bitarray, length, factor)
	...
	factorization* factorization::load(ifstream & fp){
		...
		rep->nLevels = loadValue<uint8_t>(fp);
		rep->levelsIndex = loadValue<uint>(fp, rep->nLevels+1);
		rep->rankLevels = loadValue<uint>(fp, rep->nLevels);
		rep->levels = loadValue<uint8_t>(fp, (rep->levelsIndex[rep->nLevels]/2));   
	...
	uint factorization::getSize(){
		...
		mem += sizeof(uint8_t)*(levelsIndex[nLevels]/2);
		mem += bS->getSize();
		return mem;
	}
